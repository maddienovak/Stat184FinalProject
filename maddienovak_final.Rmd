...
subtitle: "Wildfires in Portugal"
...
What variables are most correlated regarding forest fires in Portugal? 
...
ggplot(forest_fires, aes(month,temp , col=ISI)) + 
  ...
  stat_smooth() 
```
- This graphic clearly shows that the ISI is not really relevant to finding when forest fires will occur. As you can see from the month they are all dark blue and not really making a difference in the number of forest fires. This just reconfirms that Temperature plays the biggest impact on forest fires.
...
very apparent that August and Septemeber were the most likely months for fires.

***Guiding Question Answer***

The varaibles that are most correlated to forest fires in Portugal are ***Temperature***, ***DMC***, ***DC***, and ***RH*** from the forest_fires dataset.